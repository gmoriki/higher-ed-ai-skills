...
v0.6 で文書フォーマットの標準を `references/skill-format-guide.md` として策定しました（mizchi 集を参照した AI 主読 × 人間可読の両立形式）。既存 SKILL.md は半期改訂のタイミングで順次移行します。新旧混在は過渡的正常状態として許容します。
...
| [ai-tone-check](skills/ai-tone-check/) | `/ai-tone-check [draft] [role]` | 大学業務文書（広報／研修教材／学生向け案内）の AI 臭・業務文書適切性・読者負荷を subagent で 3 軸判定（v0.6 新規、top-level Claude Code セッションから起動） |
